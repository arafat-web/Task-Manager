...
<div class="container">
    ...
    <form action="{{ route('routines.update', $routine->id) }}" method="POST">
        ...
        <div class="mb-3" id="days" style="{{ $routine->frequency == 'daily' ? '' : 'display: none;' }}">
            ...
            @foreach (['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="days[]" value="{{ $day }}" id="{{ $day }}" {{ in_array($day, json_decode($routine->days, true) ?? []) ? 'checked' : '' }}>
        ...
        <div class="mb-3" id="weeks" style="{{ $routine->frequency == 'weekly' ? '' : 'display: none;' }}">
            ...
            @for ($i = 1; $i <= 52; $i++)
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="weeks[]" value="{{ $i }}" id="week{{ $i }}" {{ in_array($i, json_decode($routine->weeks, true) ?? []) ? 'checked' : '' }}>
        ...
        <div class="mb-3" id="months" style="{{ $routine->frequency == 'monthly' ? '' : 'display: none;' }}">
            ...
            @foreach (['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'] as $index => $month)
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="months[]" value="{{ $index + 1 }}" id="month{{ $index + 1 }}" {{ in_array($index + 1, json_decode($routine->months, true) ?? []) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            <input type="time" name="start_time" id="start_time" class="form-control" value="{{ $routine->start_time }}" required>
            @error('start_time')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="time" name="end_time" id="end_time" class="form-control" value="{{ $routine->end_time }}" required>
            @error('end_time')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
